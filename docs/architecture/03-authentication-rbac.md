...
> Таксономия ролей и разрешений специфицирована ниже — этот документ является единственным источником истины. Прежний TS-каркас `lib/auth/roles.ts` удалён как неиспользуемый (ни один компонент не вызывал `can()`/`roleHasPermission`); при внедрении RBAC его следует воссоздать из этой спецификации.
