...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return getRouteSeo({ locale, routeKey: "events" });
}
      ...
      <div className="flex items-end justify-between gap-4">
        ...
        <div className="hidden items-center gap-4 text-right text-[11px] text-[var(--color-ink-muted)] md:flex">
          <div className="flex flex-col items-end">
            <span className="mono text-[10px] uppercase tracking-wider opacity-70">{t("stats.total")}</span>
            <span className="mono text-[15px] font-medium tabular text-[var(--color-ink)]">{events.length}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="mono text-[10px] uppercase tracking-wider opacity-70">{t("stats.upcoming")}</span>
